...
PREPARE_LOGGING(MhalComponent_i)

MhalComponent_i::MhalComponent_i(const char *uuid, const char *label) :
    MhalComponent_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

MhalComponent_i::~MhalComponent_i()
{
}

void MhalComponent_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void MhalComponent_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &MhalComponent_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &MhalComponent_i::<callback method>)
        in the constructor.
        ...
        //Add to MhalComponent.cpp
        MhalComponent_i::MhalComponent_i(const char *uuid, const char *label) :
            MhalComponent_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &MhalComponent_i::scaleChanged);
            addPropertyListener(status, this, &MhalComponent_i::statusChanged);
        }

        void MhalComponent_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void MhalComponent_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            RH_DEBUG(this->_baseLog, "status changed");
        }
            
        //Add to MhalComponent.h
...
************************************************************************************************/
int MhalComponent_i::serviceFunction()
{
    RH_DEBUG(this->_baseLog, "serviceFunction() example log message");
    LOG_INFO(MhalComponent_i, "MHAL interface is called " << "\n");
    cout << "MHAL interface is called" << endl; 
    return NOOP;
}
